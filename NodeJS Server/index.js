...
function addPlaceFunction(req, res, query) {
	
  //Replace + and read places
  ...
  fs.readFile('places.json', function(err, content) {
      ...
      var parseJson = JSON.parse(content);
		...
		// Append to elements in placeList key and store back
   	var result = (parseJson.placeList).concat(parsedQuery);
   	parseJson.placeList = result;
      fs.writeFile("places.json", JSON.stringify(parseJson,null,4), function(err) {
			if(err) {
				throw err;
			}
